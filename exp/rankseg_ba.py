...
def rankseg_ba(output, app=2, smooth=0., allow_overlap=True, truncate_mean=False, pruning=True, verbose=0, return_tau=False):
    """
    ...
    if allow_overlap == False:
        assert output.shape[1] == 2, "The number of classes should be 2 for non-overlapping segmentation in RankSEG-BA."
        is_binary = True
    ...
    if allow_overlap == False:
        # predict = predict[:, 1, ...].reshape(batch_size, width, height)
        predict = predict.reshape(batch_size, width, height)
    else:
        predict.reshape(batch_size, num_class, width, height)
    if return_tau:
        return predict, tau_rd, cutpoint_rd
    return predict
