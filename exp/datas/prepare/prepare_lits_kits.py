...
def prepare_lits_kits(data_dir, dataset):
    print(f"Preparing {dataset} dataset in {data_dir}")
    os.makedirs(f"{data_dir}/{dataset}/train", exist_ok=True)

    if dataset == 'lits':
        src_files = glob(f"{data_dir}/{dataset}/volume-*.nii")
    elif dataset == 'kits':
        src_files = glob(f"{data_dir}/{dataset}/case_*/imaging*.nii.gz")
    ...
    for src_file in src_files:
        ...
        if not os.path.exists(os.path.join(dir, 'segmentation.nii.gz')):
            print(f"Segmentation file not found for {src_file}, it is a test file")
            import shutil
            shutil.rmtree(dir)
            continue
        make_slice(src_file, data_dir, dataset)
