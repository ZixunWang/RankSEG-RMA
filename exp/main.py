...
def get_args():
    ...
    parser.add_argument("--predict_yaml", type=str, default="argmax")
    ...
    with open(f"configs/models/{args.model_yaml}.yaml", mode="r") as model_yaml, \
         ...
         open(f"configs/predict/{args.predict_yaml}.yaml", mode="r") as predict_yaml:
        ...
        args.predict_config = yaml.load(predict_yaml, Loader=yaml.Loader)
...
def main(args):
    ...
    model = Segmentor(args.model_config, args.data_config, args.loss_config, criterion).to(device)
    ...
    if os.path.isfile(ckp_path):
        ...
        logger.info(f"Checkpoint at {ckp_path} is loaded")
    else:
        args.schedule_config["start_epoch"] = 1
        logger.info(f"No checkpoint is found")
    ...
    if args.distributed:
        ...
        model = nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank]).module
    ...
    if args.test_only:
        with torch.no_grad():
            if args.data_config["dataset"] in ["lits", "kits"] or "qubiq" in args.data_config["dataset"]:
                test_medical(model, test_loader, device, args.schedule_config["start_epoch"] - 1, args)
            else:
                test(model, test_loader, device, args.schedule_config["start_epoch"] - 1, args)
        return
    ...
    for epoch in range(args.schedule_config["start_epoch"], args.schedule_config["train_epochs"] + 1):
        ...
        train(model, train_loader, optimizer, scheduler, device, epoch, args)
        ...
        if args.main_process:
            if epoch == args.schedule_config["train_epochs"] or (args.test_config["ITER"]["test_epochs"] > 0 and epoch % args.test_config["ITER"]["test_epochs"] == 0):
                logger.info(f"Begin test epoch {epoch}")
...
if __name__ == "__main__":
    ...
    exp_name = ".".join(os.path.basename(file) for file in [args.model_yaml, args.data_yaml, args.loss_yaml, args.schedule_yaml, args.optim_yaml])
